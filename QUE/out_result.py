...
dom_df = pd.read_csv("/mnt/disk/wjh23/EaseDine/DOM/Stacking/A_data_Stacking_keyword_dolphin.txt",sep="\t")
# dom_df.loc[dom_df['dom']==-1,'dom'] = 0

# 读取推荐菜品后的数据
que_df = pd.read_csv("/mnt/disk/wjh23/EaseDineDatasets/results/Results_4_12.txt",sep="\t",header=None,names=['uuid', 'text', 'dom', 'food'])
...
print(result_df[result_df['dom']==1][result_df['food'].isna()])
print(result_df[result_df['dom']==0][~result_df['food'].isna()])

# result_df.loc[result_df['dom'] == 0, 'food']
import numpy as np
result_df.loc[result_df['dom'] == 0, 'food'] = np.nan
...
# print(result_df)
result_df.to_csv(f"{root_dir}/EaseDineDatasets/results/Results_4_13_dolphin.txt", sep='\t', header=None, index=False, na_rep='')