...
def process_data(data_df, stop_words= "DOM/stopwords/my_stopwords.txt", drop_duplicates = True):
    '''
    数据预处理：
    1.分词、去停用词
    2.去重
...
class Embeding():
    ...
    def train_lda(self, data, num_topics = 5):
        ...
        # 保存模型
        lda_model.save(f"./project/DOM/embeding_models/lda/lda_model_{num_topics}.model")
        # 保存词典（Dictionary）
        dictionary.save(f"./project/DOM/embeding_models/lda/lda_dictionary_{num_topics}.gensim")
        # 保存语料库（可选，MM 格式）
        corpora.MmCorpus.serialize(f"./project/DOM/embeding_models/lda/lda_corpus_{num_topics}.mm", corpus)
...
def based_on_keywords(data_df):
    ...
    # 点餐关键词列表
    pos_keywords = load_keywords_from_file("DOM/pos_keywords.txt")

    # 非点餐关键词列表
    neg_keywords = load_keywords_from_file("DOM/neg_keywords.txt")
    # 创建临时列
